...
Click on FIRST line - script automatically finds connected chain.
...
3. Click on the FIRST line segment (defines direction)
4. Script finds all connected lines automatically
5. Adaptive components will be placed
...
DEFAULT_FAMILIES = "Invert_segment, Haunch_right, Haunch_left, Crown, Base_segment"
...
def find_closest_segment(segments, click_pt):
    """Find closest segment to click and orient by click position"""
    if not segments:
        return None
    ...
    s0, s1 = closest
    d0 = (px - s0.X) ** 2 + (py - s0.Y) ** 2 + (pz - s0.Z) ** 2
    d1 = (px - s1.X) ** 2 + (py - s1.Y) ** 2 + (pz - s1.Z) ** 2

    return (s1, s0) if d1 < d0 else closest


def build_chain(first_seg, all_segments):
    """Build ordered chain from first segment by proximity"""
    chain = [first_seg]
    used = {id(first_seg)}

    # Mark original segment as used
    for seg in all_segments:
        s0, s1 = seg
        f0, f1 = first_seg
        if (abs(s0.X - f0.X) < 0.01 and abs(s0.Y - f0.Y) < 0.01 and abs(s1.X - f1.X) < 0.01 and abs(
                s1.Y - f1.Y) < 0.01) or \
                (abs(s0.X - f1.X) < 0.01 and abs(s0.Y - f1.Y) < 0.01 and abs(s1.X - f0.X) < 0.01 and abs(
                    s1.Y - f0.Y) < 0.01):
            used.add(id(seg))

    current_end = first_seg[1]
    ...
    for _ in range(len(all_segments)):
        ...
        for seg in all_segments:
            ...
            if d0 < best_dist:
                best_dist, best_seg, best_reverse = d0, seg, False
            if d1 < best_dist:
                best_dist, best_seg, best_reverse = d1, seg, True

        if not best_seg or best_dist > MAX_GAP_FT * MAX_GAP_FT:
            break

        used.add(id(best_seg))
        if best_reverse:
            chain.append((best_seg[1], best_seg[0]))
            current_end = best_seg[0]
        else:
            chain.append(best_seg)
            current_end = best_seg[1]
...
def main():
    ...
    first_seg = find_closest_segment(all_segs, click_pt)
    if not first_seg:
        forms.alert("No segment found!", exitscript=True)
        return

    chain = build_chain(first_seg, all_segs)
    if len(chain) < 1:
